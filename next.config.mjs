...
const nextConfig = {
  webpack: (config, { isServer }) => {
    ...
    } else if (Array.isArray(config.resolve.alias)) config.resolve.alias.push({ name: 'msw/node', alias: false });
  ...
  images: {
    remotePatterns: [
      ...
      {
        protocol: 'https',
        hostname: 'crabit-bucket-prod.s3.ap-northeast-2.amazonaws.com',
        port: '',
        pathname: '/**',
      },
    ],
