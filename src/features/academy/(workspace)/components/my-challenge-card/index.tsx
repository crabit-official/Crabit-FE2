...
function MyChallengeCard({ title, thumbnailImageUrl, endedAt, startedAt, studentChallengeId, challengeLogSubmissionStatus }: IStudentChallenge) {
  ...
  return (
    <Flex
      onClick={() => router.push(`/academy/1/my-challenge/3/${studentChallengeId}`)}
      column="between"
    ...
    >
      ...
      <Flex row="between">
        ...
        <StateLabel label={getStatusName(challengeLogSubmissionStatus)} variant={getVariantByStatus(challengeLogSubmissionStatus)} />
      </Flex>
    </Flex>
  );
}
