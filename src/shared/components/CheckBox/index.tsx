...
function CheckBox({ label, id, disabled, register, required, errors, onClickLabel }: ICheckboxProps) {
  return (
    <label className="flex w-fit items-center gap-2 pl-1">
      ...
      <Typography onClick={onClickLabel} size="h5" className={`font-normal ${errors[id] ? 'font-medium text-main-pink' : 'text-neutral-500'} `} as="p">
        {label}
      </Typography>
    </label>
  );
