...
export default function ChallengeStatusBar({ totalDays, challengeLogSubmissionStatus, hasTodayChallengeLog, startedAt }: IChallengeStatusBarProps) {
  ...
  return (
    <Flex
      row="between"
      className={`w-full items-center justify-start gap-4 rounded-lg border border-solid px-3 py-2 font-medium sm:gap-6 md:justify-between ${hasTodayChallengeLog || challengeLogSubmissionStatus === CHALLENGE_LOG_SUBMISSION_STATUS.ALL_LOGS_SUBMITTED || challengeLogSubmissionStatus === CHALLENGE_LOG_SUBMISSION_STATUS.SUBMISSION_FAILED ? 'border-none bg-gray-100 text-gray-500' : 'border-main-deep-pink/60 text-main-deep-pink'} `}
    >
